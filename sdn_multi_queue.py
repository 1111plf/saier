...
class Win(gtk.Window):
    def __init__(self):
        ...
        self.sourceDir = self.curDir + "/log/"
        self.targetDir_SFQ = self.curDir + "/log_SFQ/"
        self.targetDir_PQ = self.curDir + "/log_PQ/"
        self.targetDir_CBWFQ = self.curDir + "/log_CBWFQ/"
        self.targetDir_LLQ = self.curDir + "/log_LLQ/"
        cur_path = os.getcwd()
        if not os.path.exists(self.sourceDir):
            os.makedirs(self.sourceDir)
        if not os.path.exists(self.targetDir_SFQ):
            os.makedirs(self.targetDir_SFQ)
        if not os.path.exists(self.targetDir_PQ):
            os.makedirs(self.targetDir_PQ)
        if not os.path.exists(self.targetDir_CBWFQ):
            os.makedirs(self.targetDir_CBWFQ)
        if not os.path.exists(self.targetDir_LLQ):
            os.makedirs(self.targetDir_LLQ)
        #choose queue
	...
	c0=RemoteController(self.str_name[10], port=6653)
    ...
    def graph_client(self, tag_s2):
        ...
        for i in range(count):
            ...
            plt.figure(tag_s2[i])
            for j in range(1, 5):
                if os.path.exists(self.sourceDir+'/client_vhc'+str(tag_s2[i])+'-vhs'+str(j)+'.out'):
                    fobj=open(self.sourceDir+'/client_vhc'+str(tag_s2[i])+'-vhs'+str(j)+'.out','r')
                    data=''
                    for line in fobj:
                        data+=(str(line))
    ...
    def graph_server(self):
        ...
        for i in range(1, 5):
            fobj=open(self.sourceDir+'/server_vhc'+str(5-i)+'-vhs'+str(5-i)+'.out','r')
            data=''
            for line in fobj:
                data+=(str(line))
            pattern=re.compile(
    ...
    def graph_bandwidth(self, wdget):
        ...
        for i in range(1, 5):
            k = 221
            for a in range(4):
                fobj=open(cur_path+filedir[a]+'/server_vhc'+str(5-i)+'-vhs'+str(5-i)+'.out','r')
                data=''
                for line in fobj:
                    data+=(str(line))
                pattern=re.compile(
